  ...
  return (
    <PageWrap>
      ...
      <div className="grid grid-cols-4 gap-4">
        {STATS.map((s) => (
          mode === "mid" ? (
            <MidFiStatCard key={s.label} label={s.label} value={s.value} />
          ) : (
      ...
      <div className="grid grid-cols-[320px_1fr] gap-6">
        ...
        <Card className="p-5 flex flex-col gap-4">
          {mode === "mid" ? (
            <>
              <p className="text-xs font-semibold" style={{ color: "#6b7280" }}>สรุปตามสถานะ (24 รายการ)</p>
                ...
                {STATUS_OVERVIEW.map((s) => (
                  <div key={s.label} className="flex flex-col gap-1">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xs font-bold" style={{ color: "#6b7280" }}>{s.count}</span>
                    </div>
              ...
              <div style={{ borderTop: `1px solid ${C.border}`, paddingTop: 16, marginTop: 4 }}>
                <p className="text-xs font-semibold mb-3" style={{ color: "#6b7280" }}>งบประมาณรวมที่ขอ (ปี 2568)</p>
                <div className="flex flex-col gap-2">
                  {[
                    { label: "งบประมาณที่ได้รับ",   value: "120M บาท" },
                    { label: "ยื่นขอแล้ว",           value: "29.9M บาท" },
                    { label: "คงเหลือโดยประมาณ",    value: "90.1M บาท" },
                  ].map((r) => (
                    <div key={r.label} className="flex justify-between items-center">
                      <span className="text-xs" style={{ color: "#6b7280" }}>{r.label}</span>
                      <span className="text-xs font-semibold" style={{ color: "#4b5563" }}>{r.value}</span>
                    </div>
                  ))}
                  <div className="h-2 rounded-full mt-1 overflow-hidden" style={{ background: "#e5e7eb" }}>
                    <div className="h-full rounded-full" style={{ width: "25%", background: "#9ca3af" }} />
                  </div>
                  <span className="text-[10px]" style={{ color: "#9ca3af" }}>ใช้ไปแล้ว 25% ของงบที่ได้รับ</span>
                </div>
              </div>
            </>
          ) : (
                      ...
                      <div
                        ...
                        style={{
                          ...
                          background: s.variant === "approved" ? "#16a34a"
                            : s.variant === "returned" || s.variant === "rejected" ? "#dc2626"
        ...
        <Card className="p-5 flex flex-col gap-4">
          <div className="flex items-center justify-between">
            {mode === "mid" ? (
              <>
                <p className="text-xs font-semibold" style={{ color: "#6b7280" }}>คำขอล่าสุด</p>
                <button className="text-xs px-3 py-1 rounded" style={{ background: "#e5e7eb", color: "#6b7280" }}
          ...
          {mode === "mid" ? (
            <MidFiTable
              rows={6}
              cols={8}
              headers={["เลขที่คำขอ", "หน่วยงาน", "จำนวนรถ", "งบประมาณ", "ขั้นตอน", "วันที่", "สถานะ", ""]}
              showHeaders={true}
            />
          ) : (
            <Table
